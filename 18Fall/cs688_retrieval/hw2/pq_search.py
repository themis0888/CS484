...
def test():
	...
	for filename in feature_dict:
		temp = np.append(temp, np.expand_dims(feature_dict[filename],axis=0),axis=0)
		file_list.append(filename)

	temp = temp[1:].astype(np.float32)
	pq.fit(temp)
	X_code = pq.encode(temp)
	...
	for query in query_list:
		dist = pq.dtable(feature_dict[query]).adist(X_code)
		ind = np.argpartition(dist,4)[:5]
		top_k_list, top_k_dist = [], dist[ind]
		for i in range(4):
			top_k_list.append(file_list[ind[i+1]])
