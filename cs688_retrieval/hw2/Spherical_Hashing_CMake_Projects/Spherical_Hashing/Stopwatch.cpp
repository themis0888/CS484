...
// $Id: stopwatch.cpp,v 1.1.1.1 2006/11/29 15:58:21 seanc Exp $
//------------------------------------------------------------------------------
...
double StopwatchBase::GetTime() const
{
 ...
 */
double StopwatchBase::GetAvgTime() const
{
...
double StopwatchGeneric::diffTime() const
{
  struct timeb newStamp;
  ftime(&newStamp);
  return (double)(newStamp.time - lastStamp.time)
    + ((double)(newStamp.millitm - lastStamp.millitm))/1000.0f;
}
...
double StopwatchGTOD::diffTime() const
{
...
double StopwatchSGI::diffTime() const
{
  timespec_t newStamp;
...
double StopwatchWin32::diffTime() const
{
  LARGE_INTEGER newStamp;
  QueryPerformanceCounter(&newStamp);
  return (double)((newStamp.QuadPart - lastStamp.QuadPart)/(double)clockFreq.QuadPart);
}
#endif /* _WIN32 */
...
double CPUStopwatchGeneric::diffTime() const
{
  return (clock()-lastStamp)/double(CLOCKS_PER_SEC);
}
